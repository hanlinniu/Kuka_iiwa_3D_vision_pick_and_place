...
// align the workpiece_test.pcd with workpiece_template.pcd and get the translation and rotation matrix
    ...
    if (pcl::io::loadPCDFile<PointNT> ("workpiece_test_0.pcd", *scene) < 0 ||
        pcl::io::loadPCDFile<PointNT> ("workpiece_template_0.pcd", *object) < 0)
    {
...
// read the initial pose
     ...
     // initialize gripper
     ros::NodeHandle n;
     ros::Publisher command_pub = n.advertise<robotiq_c_model_control::CModel_robot_output>("/CModelRobotOutput", 10);
...
//broadcast the poses under different frames for check it they are calculated properly
          static_broadcaster.sendTransform(static_transformStamped);
          static_broadcaster_zero.sendTransform(static_transformStamped_zero);
          static_broadcaster_one.sendTransform(static_transformStamped_one);
          static_broadcaster_two.sendTransform(static_transformStamped_two);
 ...
 // go to initial position
          ...
          while (success_plan) {
              ...
              cout<< "if you want this motion to initial position, type Y; otherwise, type N for replanning"<<endl;
              cin >> type;
...
// go to workpiece position with high altitude
          command_cartesian_position = group.getCurrentPose(ee_link);
          ...
          command_cartesian_position.pose.position.z = translated_cartesian_position.pose.position.z;
...
// go to workpiece position with grasp altitude
          ...
          while (success_plan) {
              cout<< "if you want this motion plan after goint to grasp altitude, type Y; otherwise, type N for replanning"<<endl;
              cin>>type;
              if (type == 'y')
              {
                  enter_index = 1;
...
// go up to workpiece position with high altitude
          command_cartesian_position = group.getCurrentPose(ee_link);
          command_cartesian_position.pose.position.z = hold_altitude;
          ...
          while (success_plan) {
              cout<< "if you want this motion plan after goint to hold altitude, type Y; otherwise, type N for replanning"<<endl;
              cin>>type;
              if (type == 'y')
              {
                  enter_index = 1;
...
// go to initial position with grasp altitude
...
// go to initial position with hold altitude
