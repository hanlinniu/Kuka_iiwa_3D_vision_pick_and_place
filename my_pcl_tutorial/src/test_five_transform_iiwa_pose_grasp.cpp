...
// 8, grasp
// 9, bring to the unload position and unload it
...

{
    ...
    ros::Publisher finger_command_pub = nh.advertise<robotiq_s_model_control::SModel_robot_output>("/SModelRobotOutput", 10);

    // Create a ROS subscriber for the input point cloud
    ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2> ("/kinect2/sd/points", 1, cloud_cb);
    ros::Subscriber cartesian_pose_sub = nh.subscribe("/iiwa/state/CartesianPose", 100, chatterCallback);
   ...
   // pub = nh.advertise<pcl_msgs::ModelCoefficients> ("output", 1000);
    ...
    robotiq_s_model_control::SModel_robot_output finger_command_msg;
    ...
    // activate finger
    finger_command_msg.rACT = 1;
    finger_command_msg.rMOD = 0;
    finger_command_msg.rGTO = 1;
    finger_command_msg.rATR = 0;
    finger_command_msg.rPRA = 0;
    finger_command_msg.rSPA = 255;
    finger_command_msg.rFRA = 150;
    ...
    while (ros::ok())
    {
      ...
      finger_command_pub.publish(finger_command_msg);
      ros::spinOnce();
      ros::Duration(1).sleep();
      ...
      while (ros::ok())
      {
        cout<< "please leave the worpiece on the table, type ENTER"<<endl;
        if (cin.get() == '\n')
        {
          break;
        }
            ...
            if (align.hasConverged ())
            {
                ...
                while (ros::ok())
                {
                  if (my_iiwa.getRobotIsConnected()) 
                  {
                    ...
                    //command_cartesian_position = group.getCurrentPose(ee_link);
                    command_cartesian_position.pose.position.z = grasp_altitude-0.01;


                    cout<< "if you want to move to unload workpiece, type ENTER"<<endl;
                    ...
                    if (cin.get() == '\n')
                    {
                      ...
                      cout<< "Unload workpiece finished!"<<endl;
